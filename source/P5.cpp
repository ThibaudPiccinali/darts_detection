...
int main() {
    ...
    for (int i =1; i<14;i++){
        for (int j =1; j<3;j++){
            std::string filename_image_base = "tests/" + std::to_string(i) + "/base_image_cam"+ std::to_string(j)  +"_gray.png";
            std::string filename_image_dart = "tests/" + std::to_string(i) + "/dart_image_cam"+ std::to_string(j)  +"_gray.png";

            base_image_cam1_gray = cv::imread(filename_image_base, cv::IMREAD_GRAYSCALE);
            dart_image_cam1_gray = cv::imread(filename_image_dart, cv::IMREAD_GRAYSCALE);
            dart_image_cam1_colors = cv::imread(filename_image_dart);
            ...
            cv::imshow("Tronc avec droite filtree", filtered_image);
            cv::waitKey(0);
            cv::destroyAllWindows();
